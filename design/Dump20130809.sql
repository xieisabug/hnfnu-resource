...
CREATE TABLE `r_category` (
  ...
  `remark` text,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='类别表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `r_category` VALUES (1,'文章朗读',''),(2,'动画演示',''),(4,'数学公式',''),(5,'入门习题',''),(6,'习题解答','');
...
INSERT INTO `r_course` VALUES (1,'第一课 小妈妈找蝌蚪',1,1,'sdjk'),(2,'第四课 爷爷好烦人',3,4,NULL),(3,'第二课 小胖胖找小笨笨',2,3,''),(4,'第三课 吃南瓜粑粑',3,4,'');
...
CREATE TABLE `r_source_category_join` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8 COMMENT='资源和类别的挂接表，一个资源有多个类别。一个类别也可以属于多个资源';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `r_source_category_join` VALUES (1,9,1),(2,9,2),(3,10,1),(4,10,2),(5,10,3),(6,10,4),(7,10,5),(8,11,1),(9,11,2),(10,11,4),(11,11,5),(12,11,6),(13,12,1),(14,12,2),(15,12,4),(16,12,5),(17,12,6),(18,10,6),(19,1,1),(20,1,2),(21,1,4),(22,1,5),(23,1,6),(24,13,1),(25,13,2),(26,13,4),(27,13,5),(28,13,6),(29,14,2),(30,14,6);
...
INSERT INTO `r_subject` VALUES (1,'语文','sdfwse'),(2,'数学',''),(3,'历史',''),(4,'自然','');
...
INSERT INTO `r_topic` VALUES (1,'三笔字','三笔字为毛笔，钢笔，铅笔三笔','笨笨大王',''),(2,'美食展','萝卜青菜','胖胖喽喽','');
...
INSERT INTO `s_role_menu_join` VALUES (29,1,46,'1;2;3'),(30,1,47,'1;2;3;6'),(31,2,47,'1;2;3;6'),(32,3,49,'1;2;3'),(95,4,46,'1;2;3'),(96,4,47,'1;2;3;6'),(97,4,48,'1;2;3;6'),(98,4,49,'1;2;3'),(99,4,50,'1;2;3'),(100,4,51,'1;2;3'),(101,4,52,'1;2;3'),(102,4,53,'1;2;3'),(103,4,54,'1;2;3'),(104,4,55,'1;2;3'),(105,4,57,'1;2;3;6');
...
--
-- Temporary table structure for view `v_source_more`
--

DROP TABLE IF EXISTS `v_source_more`;
/*!50001 DROP VIEW IF EXISTS `v_source_more`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `v_source_more` (
...
--
-- Final view structure for view `v_source_more`
--

/*!50001 DROP TABLE IF EXISTS `v_source_more`*/;
/*!50001 DROP VIEW IF EXISTS `v_source_more`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_source_more` AS select `t1`.`id` AS `sourceId`,`t1`.`name` AS `sourceName`,`t1`.`courseId` AS `courseId`,`t2`.`name` AS `courseName`,`t3`.`id` AS `gradeId`,`t3`.`name` AS `gradeName`,`t4`.`id` AS `subjectId`,`t4`.`name` AS `subjectName`,`t1`.`keyWords` AS `keyWords`,`t1`.`mediaType` AS `mediaType`,`t1`.`mediaFormat` AS `mediaFormat`,`t1`.`playTime` AS `playTime`,`t1`.`fileSize` AS `fileSize`,`t1`.`author` AS `author`,`t1`.`publisher` AS `publisher`,`t1`.`description` AS `description`,`t1`.`createDate` AS `createDate`,`t1`.`approvalStatus` AS `approvalStatus`,`t1`.`quality` AS `quality`,`t1`.`price` AS `price`,`t1`.`viewTimes` AS `viewTimes`,`t1`.`useTimes` AS `useTimes`,`t1`.`url` AS `url`,`t1`.`createUserId` AS `createUserId`,`t5`.`name` AS `createUserName`,`t7`.`id` AS `categoryId`,`t7`.`name` AS `categoryName` from ((((((`r_source` `t1` join `r_course` `t2` on((`t1`.`courseId` = `t2`.`id`))) join `r_grade` `t3` on((`t2`.`gradeId` = `t3`.`id`))) join `r_subject` `t4` on((`t2`.`subjectId` = `t4`.`id`))) join `s_user` `t5` on((`t1`.`createUserId` = `t5`.`id`))) join `r_source_category_join` `t6` on((`t1`.`id` = `t6`.`sourceId`))) join `r_category` `t7` on((`t7`.`id` = `t6`.`categoryId`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2013-08-09 12:40:40
